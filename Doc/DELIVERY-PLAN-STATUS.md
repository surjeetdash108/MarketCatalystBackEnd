...
> below. It records seven changes deployed to production on 2026-08-16
> (worker `market-catalyst-backend` rev 00101, live `market-catalyst-live`
> rev 00040, UI hosting redeployed, new Cloud Run Job `premarket-job`
> created + validated)._
>
> **This doc, specifically:** No row's `%` changes — these are data-completeness,
> coverage and cost/infra improvements on already-scored rows, plus one **new
> vendor feed** (FMP news). Rows most directly affected: **R15** (news feed now
> Polygon **+ FMP**, vendor-badged), **R24** (stock detail — `/live/company` full
> fields + full detail-page set on first fetch), **R47** (earnings — full-US
> forward coverage). The G item below **supersedes the 2026-07-24 block's cost
> figures**: the honest bill was ~$75–90/mo (an always-on worker) and is now
> <$15/mo (scale-to-zero worker + a premarket Cloud Run Job).
>
> **A · FMP news (new — was previously out of scope).** The news feed now MERGES
> Polygon + FMP articles, deduped by URL (Polygon wins a collision). Every article
> carries a `vendor` field ("polygon" | "fmp"), badged in the UI alongside
> publisher `source` and `sentiment`. Backend: env `NEWS_FMP_SOURCE=fmp`, new
> `src/adapters/fmp-news.adapter.ts` + `NEWS_FMP_ADAPTER` token, a `news.job.ts`
> merge loop, and the `/live/news` on-demand path (ondemand.service.ts) also stamps
> `vendor`; FMP client `getStockNews()` hits `/stable/news/stock`. UI: commentary +
> stock render vendor + sentiment pills + thumbnails. NOTE: this deliberately
> expands FMP's scope beyond the "never news" line in FMP-INTEGRATION.md
> (redistribution licensing flagged + accepted); FMP `sentiment` is frequently null.
>
> **B · Stock-detail wiring fixes.** Root cause: both `/live/*` and `/market-data/*`
> route to the `market-catalyst-live` service, which was serving a **stale image** —
> redeployed. `/live/company` (ondemand.getCompany) now builds peRatio, eps,
> dividendYield, dividendPerShare and peers (was a slim profile-only doc). UI fixes:
> dashboard analyst popup shows price-target median/low-high + recent grades (was a
> stale "not built yet" note); commentary news thumbnails; IPO "Shares" + "Deal
> size" columns; dashboard MoverPopup null-guards RVOL/RS (was rendering fake
> "0×"/"0/99").
>
> **C · stock-history self-heal → RS/tech coverage 70 → 376/385.** A ticker whose
> deep-history fetch once failed was wrongly marked "backfilled" (earliestSyncedFrom
> set), so it never re-attempted the deep window and stayed below rs-rating's 65-bar
> minimum — ~250 tickers, incl. every mega-cap, were unscored. `stock-history.job.ts`
> now runs a cheap `count()` guard that re-triggers the deep window when a
> "backfilled" ticker still has < 65 stored bars (self-healing, self-limiting).
> rsRating coverage went 70 → 376/385 (remaining are genuine recent-IPOs with < 65
> trading days). This is the concrete answer to R25's open "wide-enough pool?"
> question.
>
> **D · On-demand completeness + shared compute.** `ondemand.getCompany`'s
...
> rating ranked against the cached universe — and persists ~2yr bars to `ohlcv_bars`
> so the nightly RS/tech-rating crons also rank the new ticker. Crons now persist
> RAW scores (`rs-rating`→`rsScore`; `tech-rating`→`techMomentum`/`techTrend`/
> `techRsi`) to enable on-demand ranking. PURE compute functions are shared by BOTH
> cron and on-demand (identical field sets): `computeIndicators`, `computeRsScore`+
> `rsPercentile`, `computeTechComponents`+`techRatingFromComponents`. Existing cron
> ranks stay authoritative; a single-ticker percentile is computed only when a
> brand-new ticker lacks one.
>
> **E · Earnings full-US coverage.** The forward (FMP) earnings calendar no longer
> filters to the ~385 tracked `companies` — it resolves every FMP calendar symbol
> against the ~13,106-row Polygon US ticker reference (`tickers`, written by
> ticker-universe.job), keeping CS/ADRC US listings with Polygon names and dropping
> FMP's worldwide rows (`earnings.job.ts loadRefNames`). Reported/historical rows
> were already full-US (Polygon). Aug 26 2026 went 4 → 38 reporters;
> `earnings_events` ~7.3k → ~8.8k. NOTE: `/market-data/earnings` still returns the
> whole collection — add date-windowed loading if it grows.
>
> **F · Nightly sync integrity (single field-owner).** `companies.job.ts` no longer
> merge-writes placeholder nulls for `beta`, `volume`, `averageVolume`,
> `week52Range` — the profile write was nulling them between runs (`averageVolume`/
> `week52Range` are dead; UI uses avgVolume20/50 and computes the 52-week range
> client-side). Field ownership is now single-owner across the nightly chain:
> companies (profile) → technical-indicators (indicators+beta+high52/low52+keyLevels)
> → rs-rating (rsRating+rsScore) → tech-rating (techRating+components+sectorRank);
> market-quotes owns price/pctChange/volume. No duplication: one `companies`
> doc/ticker, `ohlcv_bars` keyed `{ticker}_{barDate}`, `earnings_events`
> full-refresh + stale-delete.
>
> **G · Cost architecture → scale-to-zero (~$75–90/mo → <$15/mo).** The dominant
> cost was the always-on worker (`min-instances=1` + `--no-cpu-throttling` ≈ $65/mo),
> which existed ONLY to host the DETACHED premarket bundle (returns 202, keeps
> running past Cloud Run's 900s request limit). The worker `market-catalyst-backend`
> is now `--min-instances=0` + CPU throttling (scale-to-zero, billed per request;
> handles short scheduled jobs + admin, cold-starts on trigger). The premarket bundle
> became a **Cloud Run Job** `premarket-job` (same image, command `node
> dist/job-entry.js`): `src/job-entry.ts` boots a Nest context via the NEW
> `src/app-job.module.ts` (worker modules MINUS ServeStaticModule, which crashes a
> no-HTTP context) and runs `SyncRegistry.get(SYNC_JOB ?? "premarket")()` then exits
> (task-timeout 3600s, max-retries 1, 2Gi/1cpu, SA backend-runtime@, ~18 min/weekday
> ≈ $0.50/mo). Scheduler `run-premarket-job` (`0 8 * * 1-5` ET) triggers it via OAuth
> POST as `scheduler-invoker@` (granted `roles/run.invoker` on the job); the old
> `sync-premarket` scheduler was DELETED. Intraday syncs relaxed 2 min → 5 min
> (`*/5 9-16 * * 1-5`: quotes/movers/breadth/indices/fear-greed). Safe because the
> `@Cron` auto-purge/retention are gated off and the metering flush runs on
> `onModuleDestroy`/SIGTERM. Manual premarket is now `gcloud run jobs execute
> premarket-job` (NOT `POST /sync/premarket/run`, whose detached path would be killed
> on the scale-to-zero worker).
...
| R47 | W10 | 🔵 | Earnings + Macro calendars on real date ranges (structural) | FMP + Finnhub | FMP `/stable/earnings-calendar`; Finnhub `/calendar/economic` | **Earnings + Macro calendars** · date-anchored calendars | **80%** | Earnings coverage thin | **Ahead of schedule** — date-anchored calendar built (`earnings-calendar.tsx`, `calendar-range.ts`); FMP's 10-row coverage limits it. ⚠ **2026-08-03:** two structural gaps confirmed in code — (a) `earnings.job.ts:40` is **forward-only** (`from=today`, `to=+30d`, no lookback) so `epsActual`/surprises are always null; (b) the macro side uses **FRED latest prints** (`estimate:null`, release-date not forward-scheduled), not a true consensus calendar. And `FMP_API_KEY` is EMPTY (O6) → earnings side returns nothing at runtime now. To 100%: add an earnings lookback window, lift the row cap (fund FMP or second-source Finnhub), add scheduled forward macro dates + estimates. Stays **~80%**. ⬆ **2026-08-16 — earnings coverage widened (cap unchanged):** the forward calendar no longer filters to the ~385 tracked `companies` — it resolves FMP calendar symbols against the ~13,106-row Polygon US ticker reference, so it now lists **ALL US reporters** (Aug 26 2026: 4 → 38; `earnings_events` ~7.3k → ~8.8k). This closes the "thin coverage" gap in the count of names, but the row cap is unmoved: the lookback/estimate structural gaps above remain, and runtime still depends on `FMP_API_KEY` being funded (O6). See the 2026-08-16 block, item E. |
