...
function errorBody(statusCode: number, error: string, message: string, extra?: Record<string, unknown>) {
  return {
    statusCode,
    error,
    message,
...
export class SyncController {
  ...
  async jobs() {
    ...
    try {
      ...
      this.logger.warn(`sync_meta unavailable — returning registered jobs without run history: ${metaError}`);
    }
  ...
  async status() {
    try {
      ...
      throw new HttpException(errorBody(HttpStatus.SERVICE_UNAVAILABLE, 'SERVICE_UNAVAILABLE', `Firestore sync_meta read failed: ${err.message}`), HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
  ...
  async jobStatus(@Param('job') job: string) {
    try {
      ...
      throw new HttpException(errorBody(HttpStatus.SERVICE_UNAVAILABLE, 'SERVICE_UNAVAILABLE', `Firestore sync_meta read failed for "${job}": ${err.message}`), HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
  ...
  async run(@Param('job') job: string) {
    ...
    if (!runner) {
      throw new NotFoundException(`Unknown job "${job}". Available: ${this.registry.names().join(', ')}`);
    }
    try {
      return await runner();
    } catch (err) {
      if (err instanceof AllSourcesFailedError) {
        throw new HttpException(errorBody(HttpStatus.BAD_GATEWAY, 'UPSTREAM_VENDOR_ERROR', err.message, {
      ...
      throw new HttpException(errorBody(HttpStatus.BAD_GATEWAY, 'UPSTREAM_VENDOR_ERROR', `Job "${job}" failed: ${err.message}`, { retryable: true }), HttpStatus.BAD_GATEWAY);
    }
  }
