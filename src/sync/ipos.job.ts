...
export class IposJob {
  ...
  /**
   * Live-direct: fetch + shape the IPO calendar (with aftermarket enrichment)
   * WITHOUT writing Firestore, returning the exact `{id, ...data}` shape the
   * `ipos` collection read used to yield. Backs GET /market-data/ipos.
   */
