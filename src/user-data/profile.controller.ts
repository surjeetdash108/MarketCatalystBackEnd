...
export class ProfileController {
  ...
  @Get('profile')
  async get(@CurrentUser() uid: string): Promise<Record<string, unknown> | null> {
    const snap = await this.ref(uid).get();
    if (!snap.exists) return null;
    const { createdAt, updatedAt, ...rest } = snap.data() ?? {};
    return { ...rest, createdAt: toIso(createdAt) ?? null, updatedAt: toIso(updatedAt) ?? null };
  }

  @Patch('profile')
  async update(@CurrentUser() uid: string, @Body() body: InvestorProfilePatch): Promise<Record<string, unknown> | null> {
