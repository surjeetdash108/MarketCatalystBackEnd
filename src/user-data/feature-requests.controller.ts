...
export class FeatureRequestsController {
  ...
  async create(@CurrentUser() uid: string, @Body() body: { text?: string }): Promise<FeatureRequest> {
    const text = (body.text ?? '').trim();
    if (!text) throw new BadRequestException('text is required');
    if (text.length > MAX_LEN) throw new BadRequestException(`text must be ${MAX_LEN} characters or fewer`);

    const now = Timestamp.now();
    const ref = await this.firebase.firestore.collection('feature_requests').add({
      uid,
      text,
      createdAt: now,
    });
