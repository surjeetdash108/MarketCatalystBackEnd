...
const QUARANTINE_LABEL: Record<QuarantineReason, string> = {
  split:
    "a split executed between the two comparison days, so its close-to-close %change is a pre-split/post-split artifact",
  "extreme-move":
    "its close-to-close %change is beyond the plausible-move threshold",
};
