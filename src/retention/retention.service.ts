...
export class RetentionService {
  ...
  private get dryRun(): boolean {
    return (
      String(this.config.get("RETENTION_DRY_RUN", "true"))
        .trim()
        .toLowerCase() !== "false"
    );
  }
  ...
  async scheduled(): Promise<void> {
    if (!this.scheduledJobsEnabled) {
      this.logger.log(
        "retention: scheduled run skipped (ENABLE_SCHEDULED_JOBS is not true)",
      );
      return;
    }
  ...
  async runAll(): Promise<RetentionResult[]> {
    ...
    for (const rule of RETENTION_RULES) {
      try {
        ...
        this.logger.error(
          `retention failed for ${rule.collection}: ${err.message}`,
        );
        out.push({
  ...
  async previewAll(): Promise<RetentionResult[]> {
    ...
    for (const rule of RETENTION_RULES) {
      try {
        ...
        this.logger.error(
          `retention preview failed for ${rule.collection}: ${err.message}`,
        );
      }
    }
    return out;
  ...
  private async runRule(
    ...
    const matched = (
      await col.where(rule.dateField, "<", cutoff).count().get()
    ).data().count;
    ...
    this.logger.log(
      `retention: ${rule.collection} pruned ${deleted} row(s) older than ${cutoff}`,
    );
    return {
      collection: rule.collection,
      cutoff,
